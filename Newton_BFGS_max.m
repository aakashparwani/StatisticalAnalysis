...
function [x_max,f_max,iters_max] = bfgs_max(N, X, gradient_toler, function_toler, delta_toler, 
  max_iter, myfunction)
...
%
  ...
  while bGoOn
    ...
    fprintf('-----ITERATION - Maximum value search of variables----------%i.\n',Iters); 
    
    if Iters > max_iter
      break;
    end
    
    %search direction to find maximum value of function
    S = B * grad1;
    ...
    fprintf('\n-----Maximum value of variables so far-------%i.\n',X);
    X
...
%call bfgs function.
[x_max,f_max,iters_max] = bfgs_max(2, [0 0], 1e-7, 1e-7, [1e-5 1e-5], 100, 'fx1')


fprintf('------- Let us check out the final values--------------%i.\n');

fprintf("\n----Maximum value of variables-----%i.  \n",x_max);

fprintf("-----Maximum value of function is-----%i.  \n",f_max);

fprintf("-----Number of iterations to find maximum value of function-----%i.  \n",iters_max);
